    ...
    if(USART_GetITStatus(UART4, USART_IT_RXNE) != RESET)
    {
      ...
      }else
      {
        ...
        if(counter == 30)
        {
          ...
          imuData.positionV[2] = (short)((bspData.uart4RecBuffer[28]<<8) | bspData.uart4RecBuffer[29]) * 0.01f;
          dogData.imuDog++;
    ...
    if(USART_GetITStatus(USART2, USART_IT_RXNE) != RESET)
    {
      ...
      }else
      {
        ...
        if(counter == 25)
        {
          ...
          uavData.receriverValue[2] = receiverBuf[OIL_CHANNEL];
          uavData.receriverValue[3] = receiverBuf[YAW_CHANNEL];
          uavData.receriverValue[4] = receiverBuf[FUN_CHANNEL];
          uavData.receriverValue[5] = receiverBuf[SWITCH_CHANNEL];
          dogData.receiverDog++;
