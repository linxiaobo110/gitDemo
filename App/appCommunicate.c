...
//wifi模块特殊定义
  ...
  const char wifiCmdConServer[] = "AT+CIPSTART=\"TCP\",\"192.168.0.103\",8080\r\n";
...
//    }
//  }
//	wifiErrPro:fun_printf(USART3, "error  %d", wifiErr);
	
	device->enable = 1;
  device->startStatue = WIFI_ERR_NONE;
...
//      crcTemp = CRC_CalcBlockCRC((u32 *)(buffer+4), 9);
      ...
      DMA2_Stream7->NDTR = 176;
      DMA2_Stream7->M0AR = (u32)wifiBuffer;
      DMA_ClearFlag(DMA2_Stream7, DMA_FLAG_TCIF7);//查询发送需要此条
...
void appCorrectingMode(void *p_arg)
{
  ...
  CPU_TS ts;
	...
	while(DEF_ON)
	{
		uavData.uavStatue = UAV_STATUE_CORRECT;
		      uavData.motorPwm[0] =1000;
      uavData.motorPwm[1] =1000;
      uavData.motorPwm[2] =1000;
      uavData.motorPwm[3] =1000;
		if((*(ptrMem+1) == 0x57)&&(*(ptrMem) == 0xff))
		{
			OSTaskSemPost(&appData.appMainCycleTCB, OS_OPT_POST_NONE, &err);
            {
              uavData.roll_ax = *(ptrMem + 2);
              uavData.pitch_ax = *(ptrMem + 3);
              uavData.yaw_ax = *(ptrMem + 4);
							uavData.oil_ax = *(ptrMem + 5);
//							uavData.otherPara[2]=uavData.roll_ax;
//							uavData.otherPara[3]=uavData.pitch_ax;
//							uavData.otherPara[4]=uavData.yaw_ax;
//							uavData.otherPara[5]=uavData.oil_ax;
							*uavPara[34]=uavData.roll_ax;
							*uavPara[35]=uavData.pitch_ax;
							*uavPara[36]=uavData.yaw_ax;
							*uavPara[37]=uavData.oil_ax;
            } 
	}
}
	}
...
//		correctMem=ptrMem;
    ...
    //      imuData.positionV[1] = (short)((*(ptrMem+6)<<7) | *(ptrMem+7)) - 8000;
        ...
        {
          if((uavData.uavStatue == 5)||(uavData.uavStatue == 2))
          {
            choice = *(ptrMem + 2);
        ...
        else if( *(ptrMem+1) == 0x45 )
        {
          ...
          if(((uavData.uavStatue == 5)||(uavData.uavStatue == 2)) && (*(ptrMem+  2)==0x33))
          {
            //参数保存
            for(i=0;i<16;i++)
            {
    ...
    //      }
        ...
        else if( *(ptrMem+1) == 0x51 )
        {
          uavData.uavDubugMode = (UAV_DEBUG_MODE)*(ptrMem+2);
				...
				else if( *(ptrMem+1) == 0x53 )
				{
				OSTaskSemPost(&appData.appRunTestTCB,OS_OPT_POST_NONE,&err);
				}
				else if( *(ptrMem+1) == 0x55 )
				{
				OSTaskSemPost(&appData.appCorrectingModeTCB,OS_OPT_POST_NONE,&err);
				}
    }
